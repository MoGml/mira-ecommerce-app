...
const STAGING_BASE_URL = "https://a2b.runasp.net/customer/api/";
...
export async function apiFetch(
  ...
  const shouldCache =
    method === "GET" &&
    !noCacheEndpoints.some((endpoint) => path.includes(endpoint));
      ...
      try {
        ...
        try {
          ...
          if (!path.includes("GetArea")) {
            console.warn(
              "⚠️ [API_RESPONSE] Failed to parse JSON response:",
              parseError
            );
          }
        }
        ...
        if (res.ok) {
          ...
          console.log(
            `🔄 [API_RETRY] Retrying in ${delay}ms (attempt ${attempt + 1}/${
              retries + 1
            })`
          );
          await sleep(delay);
...
export async function createGuestAddress(payload: CreateGuestAddressRequest) {
  try {
    ...
    if (e && e.body && typeof e.body === "object") {
      ...
      if (
        message.toLowerCase().includes("out of service") ||
        message.toLowerCase().includes("not covered")
      ) {
...
export type CatalogResponse = {
  subCategories: SubCategory[];
  products?: Product[]; // Optional - some APIs return products
  packs?: Pack[]; // Optional - some APIs return packs directly
  page: number;
  pageSize: number;
...
export async function getAreaCoverage(
  latitude: number,
  longitude: number
): Promise<AreaCoverage | null> {
  try {
    const query = `Addresses/GetArea?lat=${encodeURIComponent(
      latitude
    ...
    if (
      res &&
      typeof res === "string" &&
      res.toLowerCase().includes("no available area")
    ) {
...
export async function getBag(): Promise<BagResponse> {
  try {
    ...
    if (e.message === "Bag is empty" || e.body?.message === "Bag is empty") {
      console.log(
        "🛒 [GET_BAG] Bag is empty (from error) - returning empty bag structure"
      );
      return {
        customerId: 0,
        addressId: 0,
...
export async function mutateBag(
  payload: MutateBagRequest
): Promise<MutateBagResponse> {
  try {
...
export async function getCheckoutDetails(): Promise<CheckoutDetailsResponse> {
  try {
    ...
    const error: any = new Error(
      e.message || "Failed to fetch checkout details"
    );
    error.status = e.status;
    error.body = e.body;
    throw error;
...
export async function bringToMyArea(
  payload: BringToMyAreaRequest
): Promise<BringToMyAreaResponse> {
  try {
    ...
    // Re-throw with additional context
    const error: any = new Error(
      e.message || "Failed to submit bring to my area request"
    );
    error.status = e.status;
    error.body = e.body;
    throw error;
